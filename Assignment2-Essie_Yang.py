...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    for m in more_temperatures:
        temperatures.append(m)
    
    for n in iot_sensor_points:
        temperatures.append(iot_sensor_points[n])
    ...
    for m in range(0, len(temperatures)):
        if ((m+1) % 5 == 0):
            samples.append(temperatures[m])
...
def bought_food(menu, user_input):
    item = menu[user_input-1]
...
def display_menu(menu):
    ...
    menu_n = len(menu)
    menu_list = [str(i) for i in range(1, menu_n+1)]
    
    menu_content(menu)
    
    
    user_input = input("Select items by pressing item number (press 'N' to exit): ")
    num_of_input = 1
    while user_input != 'N':
        if user_input in menu_list:
            bought_food(menu, int(user_input))
        else:
    ...
    print("Total Order: " + str(num_of_input))
    
    return menu_n, menu_n
